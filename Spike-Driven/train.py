...
parser.add_argument(
    "--classeval",
    default=False,
    help="add each class evaluation to validation metrics",
)
...
def main():
    ...
    _logger.info(f"Total parameters in model: {total_params:,}")
...
def validate(model, loader, loss_fn, args, amp_autocast=suppress, log_suffix=""):
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            # added for class evaluation
            if args.classeval:
                _, pred = torch.max(output, 1)  # 모델 출력에서 예측 클래스 얻기
    ...
    if args.classeval:
        if args.local_rank == 0:
            for i in range(args.num_classes):
                ...
                _logger.info(f"Class {i}: Accuracy = {acc:.4f} ({int(class_correct[i])}/{int(class_total[i])})")
